...
function createClient(port, host, options) {
  ...
  options.breakerConfig = _.defaults(options.breakerConfig, {
    statInterval: 10000,
    threshold: 0.9, // the breaker will be always closed by default
    circuitDuration: 150000,
    timeout: 5000,
    waitThreshold: 5
  });
  ...
  const brake = new Brakes((cmd, args) => new Promise((resolve, reject) => {
    ...
    args[args.length - 1] = (err, result) => {
      if(err) {
        console.log(`rejected from ${client._isMaster ? 'master' : 'slave'} shard ${client.address}`);
        return reject(err);
      }
      console.log(`resolved from ${client._isMaster ? 'master' : 'slave'} shard ${client.address}`);
